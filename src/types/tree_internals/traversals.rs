...
use super::{super::tree_arena::TreeArena, id::NodeId, node::Node};
...
impl<'a> Iter<'a> {
    fn new(arena: &'a TreeArena, node: impl Into<Option<NodeId>>) -> Self {
        let node = node.into();
...
#[derive(Clone)]
struct DoubleEndedIter<'a> {
    arena: &'a TreeArena,
    head: Option<NodeId>,
    tail: Option<NodeId>,
}

impl<'a> DoubleEndedIter<'a> {
    fn new(
        arena: &'a TreeArena,
...
macro_rules! new_iterator {
    ($(#[$attr:meta])* $name:ident, inner = $inner:ident, new = $new:expr $(,)?) => {
        ...
        #[allow(deprecated)]
        impl<'a> $name<'a> {
            pub(crate) fn new(arena: &'a TreeArena, node: NodeId) -> Self {
                let new: fn(&'a TreeArena, NodeId) -> $inner<'a> = $new;
                Self(new(arena, node))
            }
        }
    };
    ($(#[$attr:meta])* $name:ident, new = $new:expr, next = $next:expr $(,)?) => {
        ...
        impl<'a> Iterator for $name<'a> {
            ...
            fn next(&mut self) -> Option<NodeId> {
                let next: fn(&Node) -> Option<NodeId> = $next;
        ...
        #[allow(deprecated)]
        impl<'a> core::iter::FusedIterator for $name<'a> {}
    };
    ($(#[$attr:meta])* $name:ident, new = $new:expr, next = $next:expr, next_back = $next_back:expr $(,)?) => {
        ...
        #[allow(deprecated)]
        impl<'a> ::core::iter::DoubleEndedIterator for $name<'a> {
            fn next_back(&mut self) -> Option<Self::Item> {
                match (self.0.head, self.0.tail) {
                    ...
                    (None, Some(tail)) | (Some(_), Some(tail)) => {
                        let next_back: fn(&Node) -> Option<NodeId> = $next_back;
...
impl<'a> Descendants<'a> {
    pub(crate) fn new(arena: &'a TreeArena, current: NodeId) -> Self {
...
impl<'a> core::iter::FusedIterator for Descendants<'a> {}
...
impl NodeEdge {
    ...
    /// ```
    #[must_use]
    pub fn next_traverse(self, arena: &TreeArena) -> Option<Self> {
        match self {
    ...
    /// ```
    #[must_use]
    pub fn prev_traverse(self, arena: &TreeArena) -> Option<Self> {
        match self {
...
impl<'a> core::iter::FusedIterator for Traverse<'a> {}
...
impl<'a> core::iter::FusedIterator for ReverseTraverse<'a> {}
