...
macro_rules! debug_assert_triangle_nodes {
    ($arena:expr, $parent:expr, $previous:expr, $next:expr $(,)?) => {{
        if cfg!(debug_assertions) {
            super::relations::assert_triangle_nodes($arena, $parent, $previous, $next);
        }
    }};
}

pub(crate) use debug_assert_triangle_nodes;
...
/// ... -> prev -> (new)
/// ```
pub fn insert_last_unchecked(arena: &mut TreeArena, new: NodeId, parent: NodeId) {
